...
import { SOCKETS, LINE_STYLES } from "../../config/displays/leader-lines/leader-lines.js";
...
export default class LineManager {
  ...
  showLine(lineRef, options = {}) {
    ...
    if (!lineRecord || lineRecord.visible) return false;
  ...
  connect(originSocketKey, terminusSocketKey, options = {}) {
    ...
    if (!originKey || !terminusKey) return false;
  ...
  hideAllLines(effect = "none") {
    this._lines.forEach((lineRecord) => {
      if (!lineRecord.line || typeof lineRecord.line.hide !== "function") return;
      try {
  ...
  _createLineRecord(LeaderLine, lineOptions, originSocketId, terminusSocketId, index) {
    if (!LeaderLine || typeof LeaderLine.pointAnchor !== "function") {
  ...
  _normalizeSocketKey(socketKey) {
    ...
    if (this.sockets && normalized in this.sockets) return normalized;
    const withoutHash = normalized.startsWith("#") ? normalized.slice(1) : normalized;
    if (this.sockets && withoutHash in this.sockets) return withoutHash;
    return normalized;
  }
  ...
  _resolveSocketElement(socket, elementSelector) {
    ...
    if (socketScope && typeof socketScope === "object") {
      ...
      if (normalizedScope === "document") {
        ...
        if (scopeElement) scopes.push(scopeElement);
      }
    }

    if (this.root) scopes.push(this.root);
  ...
  _findLineRecord(lineRef) {
    if (typeof lineRef === "number") return this._lines[lineRef] || null;
  ...
  _applyLineClasses(line, classNames) {
    if (typeof classNames !== "string" || !classNames.trim()) return;
  ...
  _resolveLineSvgElement(line) {
    ...
    for (const ref of directRefs) {
      ...
      if (ref instanceof SVGElement) return ref;
  ...
  _bindPositionListeners() {
    this._unbindPositionListeners();
    window.addEventListener("resize", this._handleLinePositionUpdate, { passive: true });
    window.addEventListener("scroll", this._handleLinePositionUpdate, { passive: true });
    window.addEventListener("load", this._handleLinePositionUpdate, { once: true });
  }

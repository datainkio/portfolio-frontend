...
import { AWARDS_INTRO } from "../../config/ix/motion.js";
...
export function createSlideIn(view, gelManager) {
  ...
  if (gel_backing?.view) {
    ...
    tlBacking.to(
      gel_backing.view,
      { x: -12, y: 0, ease: "power3.out", duration: GEL_DUR },
      0,
    );
    tlBacking.to(
      gel_backing.view,
      { rotation: -3, ease: "power2.out", duration: GEL_DUR },
      0,
    );
  }
  ...
  if (gel_tint?.view) {
    ...
    tlTint.to(
      gel_tint.view,
      { rotation: -4, ease: "power2.out", duration: GEL_DUR },
      0,
    );
  }

...
export default class StageManager {
   ...
   * 1. Finds smooth-content container (MUST exist for ScrollSmoother)
   ...
   * - #smooth-content element MUST exist in HTML
   ...
   *
   * WARNING: If smooth-content doesn't exist, ScrollSmoother will fail.
   * If video file is missing, background video won't load but won't break site.
   */
  initView() {
    // CRITICAL: Find ScrollSmoother content container - MUST exist
    this._container = document.getElementById("smooth-content");
    if (!this._container) {
      console.error(
        "CRITICAL: smooth-content element not found - ScrollSmoother will fail"
      );
      return;
    }
   ...
   * - Effects: Enabled for opacity and parallax animations
   * - Smooth: Currently commented out (1.5 would slow scroll smoothing)
   *
   * ANIMATION COORDINATION:
   * 1. ScrollSmoother provides foundation for smooth scroll experience
   ...
   * - HTML must have smooth-wrapper and smooth-content elements
   ...
   *
   * WARNING: ScrollSmoother affects entire site scroll behavior.
   * Conflicts with other scroll libraries can cause performance issues.
   */
  initAnimation() {
    // CRITICAL: Initialize ScrollSmoother for site-wide smooth scrolling
    // This affects ALL scroll behavior across the entire site
    ScrollSmoother.create({
      wrapper: "#smooth-wrapper", // Outer scroll container - MUST exist in HTML
      content: "#smooth-content", // Inner content container - MUST exist in HTML
      // smooth: 1.5,                // Speed multiplier (higher = slower) - commented for default
      effects: true, // Enable ScrollTrigger effects like opacity and parallax
    });
 ...
 * 1. HTML must include smooth-wrapper and smooth-content elements
 * 2. Video file (sizzle.mov) must exist and be web-optimized
 * 3. CSS classes must be defined: bg-video, bg-pixelator, bg-gel-primary, bg-gel-secondary
 * 4. Section elements (main-header, biography) must exist for animation triggers
 *
 * To debug StageManager issues:
 * - Check browser console for ScrollSmoother initialization errors
 ...
 * - ScrollSmoother affects entire site scroll performance
 ...
 * - ScrollSmoother affects all scroll-based animations site-wide
 *

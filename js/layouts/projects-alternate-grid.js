 ...
 * ---
 */
const DUR = 0.25; // base value for animation duration (in seconds)
...
function onMouseOver(e) {
  ...
  PROJECTS.forEach((project) => {
    var opacity = project != e.currentTarget ? 0.5 : 1;
    gsap.to(project, { duration: 0.5, opacity: opacity });
  });
}
...
function onMouseOut(e) {
  ...
  PROJECTS.forEach((project) => {
    gsap.to(project, { duration: 0.5, opacity: 1 });
  });
}
...
function navigate_projects(e) {
  ...
  var incoming = gsap.utils.toArray(
    "#modal_" + siblings[index + distance]["id"],
  )[0];
  outgoing.close();
  incoming.showModal();
}

...
function updateStatusTexts(lastEvent, kind = "") {
  if (ui.statusActive)
    ui.statusActive.textContent = state.hero ? "Hero ready" : "Idle";
  if (ui.statusLast) ui.statusLast.textContent = `Last: ${lastEvent || "–"}`;
  if (ui.activeModeChip)
    ui.activeModeChip.textContent = state.bus ? "Bus: active" : "Bus: idle";
  if (ui.logSummary)
    ui.logSummary.textContent = `${state.eventCount} events${kind ? " • " + kind : ""}`;
}

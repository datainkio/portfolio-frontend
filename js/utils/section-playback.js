...
function ensureInstance(sectionId) {
  ...
  try {
    ...
    console.warn(
      `[section-playback] Failed to init section ${sectionId}`,
      error,
    );
    return null;
  }
...
function setState(
  ...
  state,
  statusText,
  { introBtn, outroBtn, statusEl } = {},
) {
  el.dataset.state = state;
  if (statusEl)
    statusEl.textContent = statusText ?? STATUS[state] ?? STATUS.idle;
...
function bindControl(controlEl) {
  ...
  if (!sectionId || !introBtn || !outroBtn || !statusEl) {
    setState(controlEl, "disabled", STATUS.disabled, {
      introBtn,
      outroBtn,
      statusEl,
    });
  ...
  if (!instance) {
    setState(controlEl, "disabled", "Section not available", {
      introBtn,
      outroBtn,
      statusEl,
    });
  ...
  const updateIdle = (message) =>
    setState(controlEl, "idle", message ?? STATUS.idle, {
      introBtn,
      outroBtn,
      statusEl,
    });
  ...
  introBtn.addEventListener("click", () => {
    setState(controlEl, "intro", STATUS.intro, {
      introBtn,
      outroBtn,
      statusEl,
    });
    try {
      ...
      setState(controlEl, "disabled", STATUS.error, {
        introBtn,
        outroBtn,
        statusEl,
      });
  ...
  outroBtn.addEventListener("click", () => {
    setState(controlEl, "outro", STATUS.outro, {
      introBtn,
      outroBtn,
      statusEl,
    });
  ...
  if (events.introComplete) {
    unsubscribers.push(
      bus.on(events.introComplete, () => updateIdle("Intro complete")),
    );
  }

  if (events.outroComplete) {
    unsubscribers.push(
      bus.on(events.outroComplete, () => updateIdle("Outro complete")),
    );
  }
...
function init() {
  const controls = Array.from(
    document.querySelectorAll("[data-section-playback]"),
  );

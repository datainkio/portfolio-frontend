...
const EXCLUDE_PATTERNS = ['**/node_modules/**', '**/_site/**', '**/dist/**', '**/build/**'];
...
files.forEach((inputFile, index) => {
  ...
  try {
    console.log(chalk.cyan(`  [${index + 1}/${files.length}] Converting: ${inputFile}`));
    ...
    if (error.killed) {
      console.error(chalk.red(`    Timeout: Process killed after 30 seconds\n`));
    } else {

...
function checkNodeEnvironment() {
  ...
  healthCheck("Node.js Version", () => {
    ...
    if (major >= 18) {
      ...
      return {
        status: "fail",
        message: `${version} - requires Node.js 18+ for ES modules`,
      };
    }
  });
    ...
    () => {
      ...
      if (!value) {
        return {
          status: "fail",
          message:
            "Missing - set FIGMA_TOKEN (or legacy FIGMA_ACCESS_TOKEN) in .env file",
        };
      }
      const masked =
        value.length > 8
...
function checkFileSystem() {
  ...
  healthCheck("_site Directory", () => {
    ...
    if (!existsSync(sitePath)) {
      return {
        status: "warn",
        message: "Build directory missing - run npm run build",
      };
    }
...
function generateHealthReport() {
  ...
  if (percentage >= 90) {
    ...
    console.log(
      chalk.yellow("Address critical issues first, then run: npm run doctor"),
    );
  }
}

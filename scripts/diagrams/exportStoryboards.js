...
export async function exportStoryboards() {
  ...
  for (const file of mdFiles) {
    ...
    logger.trace('Storyboards', `Found ${blocks.length} diagram(s) in ${file}`, 'brief');
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      try {
        ...
        logger.trace('Storyboards', `Failed ${outName}: ${err.message}`, 'brief', 'error');
      }
    }
  }
  ...
  if (total === 0) {
    ...
    logger.trace('Storyboards', `Export complete: ${total} SVG(s)`, 'brief', 'success');
  }
}

...
export class DesignFile {
    ...
    log() {
        ...
        console.log(chalk.gray("\t   ↳ fills: ") + chalk.cyan(Object.keys(this.fillStyles).length));
        console.log(chalk.gray("\t   ↳ text: ") + chalk.cyan(Object.keys(this.textStyles).length));
        console.log(chalk.gray("\t   ↳ grids: ") + chalk.cyan(Object.keys(this.gridStyles).length));
        console.log("\n");
    }
    ...
    get componentSets() { return this._componentSets; }
    ...
    get styleTypes() {
        return [...new Set(
            Object.values(this._styles)
                .map(style => style.styleType)
        )];
    }

    get fillStyles() {
        return Object.fromEntries(
            Object.entries(this._styles)
    ...
    get textStyles() {
        return Object.fromEntries(
            Object.entries(this._styles)
                .filter(([_, style]) => style.styleType === 'TEXT')
        );
    }
    ...
    get gridStyles() {
        return Object.fromEntries(
            Object.entries(this._styles)
                .filter(([_, style]) => style.styleType === 'GRID')
        );
    }
    ...
    set componentSets(value) {
        if (!Array.isArray(value)) throw new Error('ComponentSets must be an array');

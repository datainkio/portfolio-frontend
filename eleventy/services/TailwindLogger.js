...
export class TailwindLogger {
  ...
  logFileAnalysis(inputPath) {
    try {
      ...
      const fontImports = imports.filter((imp) =>
        imp.includes("fonts.googleapis.com"),
      );
  ...
  analyzePerformance(cssContent) {
    ...
    const complexSelectors = selectorComplexity.filter(
      (sel) => sel.split(/[\s>+~]/).length > 4,
    );
    ...
    const hasEmptyLayers =
      cssContent.includes("@layer") && cssContent.includes("@layer{}");
    if (hasEmptyLayers) {
      tailwindLogger.trace(
  ...
  async logConfigAnalysis(configPath) {
    try {
      ...
      const corePlugins = config.corePlugins
        ? Object.keys(config.corePlugins).length
        : "default";
      ...
      tailwindLogger.trace(
        `Core plugins config: ${corePlugins}`,
        null,
        "brief",
        processStyle,
      );
  ...
  completeBuild(success = true, metrics = {}) {
    ...
    if (success) {
      ...
      if (metrics.warnings) {
        tailwindLogger.trace(
          `Build warnings: ${metrics.warnings}`,
          null,
          "brief",
          metricStyle,
        );
  ...
  logError(error, context = "") {
    ...
    if (
      ...
      errorMessage.includes("unknown at rule")
    ) {
      tailwindLogger.trace(

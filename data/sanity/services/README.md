...
- `fetchSanityData` answers: *how do I get one query's data from Sanity?*
- `sanityService` answers: *how do I wire all CMS data into an Eleventy build?*
...
- Domain-specific service files earn their place only if fetch *behavior* must vary per domain (e.g., authenticated requests, different cache duration). In that case, delegate to `fetchSanityData` rather than reimplement it.

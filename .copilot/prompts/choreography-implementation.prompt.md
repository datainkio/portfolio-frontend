...
- Config/constants-first edits: for choreography work, consume selectors/timings/settings from `frontend/js/choreography/config.js` and event names from `frontend/js/choreography/constants.js` instead of hardcoding values in section/triggers/sequence files.
- Consolidation rule: when new choreography tunables or event contracts are needed, add them to `config.js`/`constants.js` first, then import and use them at call sites.
